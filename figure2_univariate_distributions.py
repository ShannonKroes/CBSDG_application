...
dattime = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[1], 3), synth_total.T[1]]).T)
)
datage = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[2], 3), synth_total.T[2]]).T)
)
datmonth = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[3], 3), synth_total.T[3]]).T)
)
datlastfer = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[4], 3), synth_total.T[4]]).T)
)
dattimetofer = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[5], 3), synth_total.T[5]]).T)
)
dathbprev1 = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[6], 3), synth_total.T[6]]).T)
)
dattimetoprev1 = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[7], 3), synth_total.T[7]]).T)
)
datHb = get_cols(
    pd.DataFrame(np.vstack([np.repeat(data_or.T[8], 3), synth_total.T[8]]).T)
)

datsex_counts = np.zeros((2, 2))
datsex_counts[:, 0] = data_original["Sex"].value_counts().to_numpy() / len(
    data_original["Sex"]
)
...
datmonth_counts[:, 1] = (
    datmonth["Synthetic data"].value_counts().sort_index().to_numpy()
)
...
datHb_counts[:, 0] = data_original["HbOK"].value_counts().to_numpy() / len(
    data_original["HbOK"]
)
...
dattimetoprev1.plot.density(
    linewidth=1, legend=False, ax=axes[0, 1], alpha=1, style=["b-", "r--"]
)
datage.plot.density(
    linewidth=1, legend=False, ax=axes[0, 2], alpha=1, style=["b-", "r--"]
)
...
dattime.plot.density(
    linewidth=1, legend=False, ax=axes[1, 0], alpha=1, style=["b-", "r--"]
)
datlastfer.plot.density(
    linewidth=1, legend=False, ax=axes[1, 1], alpha=1, style=["b-", "r--"]
)
dattimetofer.plot.density(
    linewidth=1, legend=False, ax=axes[1, 2], alpha=1, style=["b-", "r--"]
)
...
datmonth_counts.plot.bar(
    linewidth=1, legend=False, ax=axes[2, 1], alpha=1, color=colors
)
