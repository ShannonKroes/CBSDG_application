...
for i in range(250):
    ...
    privacy_an= PoAC_and_proximity_mspn(data=data_np, inds=np.array([int(i*1000), int((i+1)*1000)]), mspn=mspn, ordered=ordered, no_tests=1000)
    save_object(privacy_an, 'results/privacy_batch_train_'+str(i))

print("working on last batch")
privacy_an= PoAC_and_proximity_mspn(data=data_np, inds=np.array([int(250000), int(250729)]), mspn=mspn, ordered=ordered, no_tests=729)
