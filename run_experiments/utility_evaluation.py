...
"""
from src.required_functions import do_svm_no_accuracy, prep_data2021, save_object, calc_shap
import pandas as pd
import numpy as np
...
ds_context=Context(meta_types=np.array([MetaType.DISCRETE, MetaType.REAL, MetaType.REAL , MetaType.DISCRETE, MetaType.REAL,MetaType.REAL, MetaType.REAL, MetaType.REAL ,  MetaType.DISCRETE]))
...
for n_i, n_ in enumerate(ns):
    for rep in range(reps):
        ...
        mspn = learn_mspn(data_np, ds_context, min_instances_slice=data_np.shape[0]-1, rand_gen=int(2206+rep), rows="kmeans", threshold=-1, standardize=True, no_clusters=4000)    
        # Generate synthetic data.
        synth_current = sample_instances(mspn, np.array([[np.repeat(np.nan,d)] * int(n_)]).reshape(-1, d), RandomState(2206+rep))
        ...
        calc_shap(data_original, clf, name="rep"+str(rep)+"_n_"+str(n_i), sample_size=100)        
        ...
        synth_predictions_2021[n_i][rep] = np.concatenate([predicted_synth_F_2021,predicted_synth_M_2021])
